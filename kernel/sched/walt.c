...
unsigned int sysctl_sched_dynamic_ravg_window_enable = (HZ == 125);
				...
				loff_t *ppos)
{
	...
	if (write && (HZ != 125 || !sysctl_sched_dynamic_ravg_window_enable))
		goto unlock;
...
{
	if (HZ == 125 && sysctl_sched_dynamic_ravg_window_enable) {
		if (fps > FPS90)
